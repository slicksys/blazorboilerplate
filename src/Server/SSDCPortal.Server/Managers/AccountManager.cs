...
namespace SSDCPortal.Server.Managers
{
    ...
    public class AccountManager : IAccountManager
    {
        ...
        {
            var requireConfirmEmail = Convert.ToBoolean(_configuration["SSDCPortal:RequireConfirmedEmail"] ?? "false");
        ...
        public async Task<ApiResponse> Create(RegisterViewModel parameters)
        {
            ...
            if (Convert.ToBoolean(_configuration["SSDCPortal:RequireConfirmedEmail"] ?? "false"))
            {
                try
                {
                    // For more information on how to enable account confirmation and password reset please visit http://go.microsoft.com/fwlink/?LinkID=532713
